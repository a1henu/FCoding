...
- Final task results are sent as cards with paginated output expand/collapse callbacks backed by in-memory runtime card state.
- GitHub Actions runs `npm ci` and `npm test` on push and pull request.
- Tests are fast and local: currently 59 Node tests.
