...
- `cards.js`: interactive card builders for command results, task status, callback tests, and expandable output cards.
...
- Editing expand/collapse card callback handling; it depends on `runtimeState.getCardState()` and 24-hour in-memory card state.
