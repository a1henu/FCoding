...
This directory owns all Feishu-specific behavior: inbound event parsing, long connection dispatch, HTTP callback crypto helpers, outbound Feishu API calls, and interactive card payloads.
...
- `cards.js`: interactive card builders.
- `crypto.js`: HTTP webhook signature verification and encrypted payload decryption.

## Internal Dependencies

- `ws.js` depends on `events.js`, `cards.js`, `../server.js`, and `../codex/runner.js`.
...
- Adding new Feishu API client methods in `client.js` with mocked fetch tests.
